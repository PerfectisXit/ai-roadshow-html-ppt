...
>
> 注：本文件负责“历史修复基线”，优先解决旧问题。
> 2026-03-13 之后，页面视觉母版、标题体系、页码位置、版式弹性、留白处理策略，以 `README_AI接手指南.md` 与 `ai-roadshow-s21-master.html` 为准。
> 如果本文件中的旧细则与母版规则冲突，应优先遵循新的母版规则。
> 本文件不单独决定分页策略、单页视觉重心或讲演节奏；这些应先按 README 中的章节级 / 单页级前置流程判断。
> 新增硬规则：页面中不得出现任何只对设计者 / 讲者 / AI 有意义的注释型文字；不得为了饱满而在同一页重复表达同一层内容；当内容较少时，优先放大重点并使用有意义的动效组件，不要为了守双栏或凑篇幅而补无效文字。若页面仍显得空，先检查主组件是否真正贴着该页讲演意图，再看组件内部和其他关键区块是否还能做实。任何新增组件都必须有讲演价值；若不帮助理解、证明或收束，应直接删除。卡片重叠、文字压边、容器遮挡是红线；在这类问题消失前，该页不得视为可交付成果。图标、logo、内嵌 SVG 图形也是高优先级手段，不得只靠放大空卡片来消化留白。
> 维护性补充：后续定位页面、review 或修样式时，以稳定 `slide id` 为唯一真源；不要再人工猜“第 N 页大概对应哪个 sXX”。新增主题修复优先收成语义类，不要继续堆末尾页级补丁。
...
页码只要求保留在幻灯片内部安全区；其具体位置与视觉样式以后续母版为准。  
...
后续精修中，如标题已采用“案例名 / 人名并入主标题”的母版写法，则可以不再单独显示 kicker。
...
<!-- 删除：操作说明 -->
<div class="cover-hint">按空格键开始 · 页码位于幻灯片内部安全区 · 所有内容固定居中展示</div>
```
